...
const WIDGET_CONFIGURATIONS = {
	...
	callbackCalendarApp: 'googlecalendar',
...
let firstCell = true;
	...
	Object.keys(events).forEach(day => {
		...
		drawDayEventsStack(dayEventsStack, day, dayEvents, maxNumAllDayEvents, hourLabels, firstCell, WIDGET_CONFIGURATIONS);
	});
}
 ...
 */
function drawDayEventsStack(stack, day, events, maxNumAllDayEvents, hourLabels, firstCell, {
	defaultTextColor,
	font,
	fontBold,
	dayOfWeekTextSize,
	...
	let halfHourEventHeight = 0;
	if (WIDGET_CONFIGURATIONS.showDayevents) {
		halfHourEventHeight = (widgetHeight - verticalSpacing - maxNumAllDayEvents) / (numHours * 2);
	}else{
		halfHourEventHeight = (widgetHeight) / (numHours * 2);
	}
	...
	const offsetY = 3;
	if (WIDGET_CONFIGURATIONS.showDayevents && events['all-day']) {
	...
	if(color==='#000000')eventsTextColor=new Color("#ffffff");
	 ...
	 offsetY = (maxNumAllDayEvents + 1) * allDayEventHeight + verticalSpacing + maxNumAllDayEvents;
	}
	...
	if (firstCell && hourLabels.includes(currentHourLabel)) {
...
function drawHoursLabelStack(stack, maxNumAllDayEvents, hourLabels, {
	...
	if (WIDGET_CONFIGURATIONS.showDayevents) {
		...
		halfHourEventHeight = (widgetHeight-20) / (numHours * 2);
	}
	const allDayEventHeight = halfHourEventHeight / 2;
	...
	for (let i = 0; i < hourLabels.length; i++) {
		...
		const topPointY = offsetY + halfHourEventHeight * 2 * i * 1.14 /*- (i * lineHeight)*/ ;

		// Draw hour text
		draw.setFont(new Font(font, eventTextSize*1));
	...
	for (let i = 0; i < numDays; i++) {
		...
		for (let i = 0; i < dayEvents.length; i++) {
			...
			if (e.isAllDay) { // All day events
				...
				if(eventDuration<=60) eventDuration = 60;
